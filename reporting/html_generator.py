...
def generate_html_report(week, season, output_dir):
    ...
    <body>
        <div class="container">
            <h1>🏈 Fantasy Projections: Week {week}, {season}</h1>
...
def generate_position_section(df, pos, top_n):
    ...
    for idx, row in subset.iterrows():
        ...
        if upside > 10: upside_str = f'<span class="boom-high">{upside_str} 🚀</span>'
...
def generate_ros_report(df, season, start_week, output_dir):
    print(f"Generating ROS report for Season {season} Week {start_week}+...")
    
    # Prepare DF: rank by ros_mean
    df = df.sort_values('ros_mean', ascending=False)
    
    html_content = f"""
    <!DOCTYPE html>
    <html>
    <head>
        <title>ROS Projections {season} Week {start_week}+</title>
        <style>
            body {{ font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif; margin: 0; padding: 20px; background: #f5f5f7; color: #333; }}
            .container {{ max_width: 1200px; margin: 0 auto; background: white; padding: 40px; border-radius: 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); }}
            h1 {{ text-align: center; color: #1d1d1f; }}
    ...
    <body>
        <div class="container">
            <h1>🔮 Rest of Season Projections: Week {start_week}-18, {season}</h1>
            <p style="text-align: center;">Average Points Per Game (Half-PPR)</p>

            {generate_ros_section(df, "QB", 20)}
            {generate_ros_section(df, "RB", 40)}
            {generate_ros_section(df, "WR", 50)}
            {generate_ros_section(df, "TE", 20)}
            {generate_ros_section(df, "K", 12)}
            {generate_ros_section(df, "DEF", 12)}

            <div class="meta">
                Generated by FantasyFootballProjections Engine v416
            </div>
        </div>
    </body>
    </html>
    ...
    output_path = os.path.join(output_dir, "ros_report.html")
    with open(output_path, "w") as f:
        f.write(html_content)
    print(f"ROS Report saved to: {output_path}")

def generate_ros_section(df, pos, top_n):
    subset = df.loc[df.position == pos].head(top_n)
    ...
    for idx, (_, row) in enumerate(subset.iterrows()):
        ...
        <tr>
            ...
            <td class="{tier_class}">{row['player_name']}</td>
            <td>{row['team']}</td>
            <td>{row['ros_mean']:.1f}</td>
        </tr>
        """
        
    return f"""
    <h2>{pos} Rest of Season</h2>
    <table>
        <thead>
            <tr>
                <th width="50">Rank</th>
